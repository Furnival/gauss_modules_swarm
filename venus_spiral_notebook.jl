...
# ╔═╡ fea2d1e8-59ca-11f1-b46e-2d54837cf636
using DifferentialEquations, Plots, PlutoUI

# ╔═╡ fea17d84-59ca-11f1-af4c-b5bebbda5dfa
md"""
# The Earth-to-Venus Spiral

*Companion notebook to "Venus, not Earth: Air-breathing satellites will fly first over Venus."*

This notebook lets you see what a low-thrust spiral trajectory from Earth's orbit to Venus's orbit actually looks like — and what changes when you turn the thrust knob up or down. The math is simple planar two-body heliocentric dynamics with a tangential thrust term, integrated forward in time until the spacecraft reaches Venus's orbital radius.

Drag the sliders. Watch the cruise time and the number of revolutions change. The trade you are looking at is the entire economic case for low-thrust interplanetary missions: more thrust means a shorter cruise but a bigger spacecraft and more propellant. Less thrust means months of patience, but a tenfold improvement in delivered mass to Venus orbit.

The default value (0.30 mm/s² on a 100 kg spacecraft) corresponds to a single Rocket Lab Gauss-class Hall thruster — about 30 millinewtons of thrust, or roughly the weight of six paperclips. The slider extends down to a hypothetical micro-thruster and up to the larger Psyche-class units flying today.
"""

# ╔═╡ fea2d774-59ca-11f1-a400-fda6fa6d45e2
md"""
## The interactive plot

**Thrust acceleration (mm/s² applied to the spacecraft):**

\$(@bind a_mm_s2 Slider(0.03:0.01:1.0, default=0.30, show_value=true))

**Initial spacecraft mass (kg):**

\$(@bind m0 Slider(50:10:500, default=100, show_value=true))
"""

# ╔═╡ ff297bf8-5bb5-43c4-b132-cb730c08fcaf
s2 = @bind a_mm_s2 Slider(0.03:0.01:1.0, default=0.30, show_value=true)

# ╔═╡ cb7c5cf0-e9ab-4eab-ad61-946c927fba9e
m0 = @bind m0 Slider(50:10:500, default=100, show_value=true)

# ╔═╡ fea2dc06-59ca-11f1-9920-f5ba990acee8
begin
    # Constants (SI units)
    μ_sun = 1.32712440018e20      # m³/s²
    AU = 1.495978707e11            # m
    r_earth = 1.000 * AU
    r_venus = 0.723 * AU
    v_earth = sqrt(μ_sun / r_earth)
    v_venus = sqrt(μ_sun / r_venus)
end

# ╔═╡ fea2e142-59ca-11f1-9c8b-fdb3159a52b7
md"""
**Acceleration setting:** \$(a_mm_s2) mm/s²  (\$(a_mm_s2 * 1e-3) m/s²)

**Spacecraft mass:** \$(m0) kg
"""

# ╔═╡ fea2e5be-59ca-11f1-b123-e5d7a6ec875b
begin
    a_thrust = a_mm_s2 * 1e-3   # m/s²
    ...
    function eom!(du, u, p, t)
        ...
        ax_g = -μ_sun * x / r^3
        ay_g = -μ_sun * y / r^3
        # Tangential thrust, aligned opposite to velocity to spiral INWARD
        # (Earth is faster than Venus; we need to decelerate)
        ax_t = -a_thrust * vx / v
        ay_t = -a_thrust * vy / v
        du[1] = vx
        du[2] = vy
    ...
    # Initial condition: circular orbit at Earth's radius
    u0 = [r_earth, 0.0, 0.0, v_earth]
    tspan = (0.0, 5.0 * 365.25 * 86400.0)  # up to 5 years

    # Stop when we reach Venus orbit radius
    condition(u, t, integrator) = sqrt(u[1]^2 + u[2]^2) - r_venus
...
# ╔═╡ fea2ea34-59ca-11f1-8ee2-69e1d7320bf9
begin
    xs = [u[1]/AU for u in sol.u]
    ys = [u[2]/AU for u in sol.u]

    θ = range(0, 2π, length=200)
    earth_orbit_x = cos.(θ); earth_orbit_y = sin.(θ)
    venus_orbit_x = 0.723 .* cos.(θ); venus_orbit_y = 0.723 .* sin.(θ)

    p1 = plot(earth_orbit_x, earth_orbit_y, color=:blue, linestyle=:dash, label="Earth orbit", aspect_ratio=:equal)
    plot!(p1, venus_orbit_x, venus_orbit_y, color=:orange, linestyle=:dash, label="Venus orbit")
    plot!(p1, xs, ys, color=:black, linewidth=2, label="Spacecraft")
    scatter!(p1, [0], [0], color=:yellow, markersize=8, label="Sun")
    scatter!(p1, [xs[1]], [ys[1]], color=:blue, markersize=4, label="Start (Earth)")
    scatter!(p1, [xs[end]], [ys[end]], color=:orange, markersize=4, label="End (Venus orbit)")
    xlabel!(p1, "x (AU)"); ylabel!(p1, "y (AU)")
    title!(p1, "Spiral trajectory: a = $(a_mm_s2) mm/s²")
    p1
end

# ╔═╡ fea2eed2-59ca-11f1-9854-5f57dd6043c5
begin
    cruise_days = sol.t[end] / 86400.0
    cruise_months = cruise_days / 30.44
    revs = 0.0
    for i in 2:length(sol.t)
        θ1 = atan(sol.u[i-1][2], sol.u[i-1][1])
        θ2 = atan(sol.u[i][2], sol.u[i][1])
        dθ = θ2 - θ1
        if dθ > π; dθ -= 2π; end
        if dθ < -π; dθ += 2π; end
        global revs += dθ
    ...
    revs = abs(revs) / (2π)
    edelbaum_days = abs(v_earth - v_venus) / a_thrust / 86400.0
#=
    md"""
 **Cruise time (numerical):** $(round(cruise_days, digits=0)) days  ($(round(cruise_months, digits=1)) months)

**Revolutions completed:** $(round( global revs, digits=2))

**Edelbaum approximation:** $(round(edelbaum_days, digits=0)) days

**Thrust on the bus:** $(round(a_thrust * m0 * 1000, digits=1)) mN (roughly $(round(a_thrust * m0 * 1000 / 9.81 / 0.5, digits=0)) paperclips' weight)

**Δv accumulated (Edelbaum):** $(round(abs(v_earth - v_venus)/1000, digits=2)) km/s
"""
=#
end

# ╔═╡ fea2f33c-59ca-11f1-8e21-4f2b806b6f16
md"""
Notice what changes as you move the slider:

- At very low thrust (around 0.05 mm/s²), the spacecraft makes four or more full revolutions before falling far enough to reach Venus, and the cruise stretches past three years.
- At the default Gauss-class thrust (0.30 mm/s² on a 100 kg bus, about 30 mN), you get a cruise of roughly five to six months and about half a revolution. This is the working point the essay describes. Real Gauss-class missions would add Earth escape and Venus capture spirals on either end, extending total mission duration to nine to twelve months.
- At higher thrust (above 0.6 mm/s²), the spiral collapses toward something resembling a Hohmann transfer arc. This is the Psyche regime — higher thrust, larger spacecraft, faster cruise, but the propellant mass fraction is rising fast.

The whole argument for low-thrust to Venus lives in that middle range.
"""

# ╔═╡ fea2f8ee-59ca-11f1-bf27-2d68b875d765
md"""
## What the satellite would actually breathe

The other half of the Venus argument is *what is in the air*. The chart below compares the upper atmospheres of Venus and Earth at the altitudes where an air-breathing satellite would scoop — roughly 150 km above Venus, 200 km above Earth.

Venus is overwhelmingly carbon dioxide, even at the top of the thermosphere. Earth at the same kind of altitude is overwhelmingly *atomic oxygen* — the species that has tormented every Earth air-breathing concept since GOCE. CO₂, by contrast, is benign: the Georgia Tech 2025 demonstration ran a Hall thruster on pure CO₂ for ten hours with no measurable electrode oxidation. The propellant is not the obstacle. The propellant is the gift.
"""

# ╔═╡ fea2fd80-59ca-11f1-8049-010c61ded7ac
begin
    venus_species = ["CO₂", "N₂", "CO", "O", "He"]
    venus_fracs   = [0.957, 0.032, 0.006, 0.003, 0.002]
    earth_species = ["O (atomic)", "N₂", "He", "O₂", "Ar"]
    earth_fracs   = [0.83, 0.12, 0.03, 0.015, 0.005]

    venus_colors = [:saddlebrown, :steelblue, :tan, :olivedrab, :goldenrod]
    earth_colors = [:olivedrab, :steelblue, :goldenrod, :seagreen, :rosybrown]

    atmo_plot = plot(xlim=(0, 1), ylim=(0, 3),
        xlabel="Number fraction",
        title="What an air-breathing satellite would scoop",
        legend=false, size=(820, 360),
        yticks=([1, 2], ["Earth ~200 km", "Venus ~150 km"]),
        framestyle=:box, gridalpha=0.2)

    left = 0.0
    for (s, f, c) in zip(venus_species, venus_fracs, venus_colors)
        plot!(atmo_plot, [left, left+f, left+f, left, left], [1.75, 1.75, 2.25, 2.25, 1.75],
              seriestype=:shape, color=c, linewidth=0.5, linecolor=:white)
        if f > 0.05
            annotate!(atmo_plot, left + f/2, 2.0, text("$s\n$(round(f*100,digits=1))%", :white, 9, :center))
        end
        left += f
    end

    left = 0.0
    for (s, f, c) in zip(earth_species, earth_fracs, earth_colors)
        plot!(atmo_plot, [left, left+f, left+f, left, left], [0.75, 0.75, 1.25, 1.25, 0.75],
              seriestype=:shape, color=c, linewidth=0.5, linecolor=:white)
        if f > 0.05
            annotate!(atmo_plot, left + f/2, 1.0, text("$s\n$(round(f*100,digits=1))%", :white, 9, :center))
        end
        left += f
    end
...
# ╔═╡ fea301fe-59ca-11f1-ad9b-05c9426af223
md"""
*Sources: Hedin VTS3 / Pioneer Venus OIMS for Venus; NRLMSISE-00 climatology for Earth. Fractions are illustrative averages over the scooping band.*

Read the two bars together. The Earth bar is mostly atomic oxygen — the corrosion problem. The Venus bar is mostly CO₂ — a propellant that Hall thrusters can ingest without measurable damage. That single visual difference is the technical reason the first air-breathing satellites will fly at Venus rather than Earth.

— Lawrence Furnival, 27 May 2026
"""
